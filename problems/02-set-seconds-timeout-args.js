...
function setSecondsTimeoutArgs(callback, seconds, ...args) {
  const delay = seconds * 1000; // Calculate the delay in milliseconds
  setTimeout(() => {
      callback(...args); // Execute the callback function with provided arguments after the delay
  }, delay);
}
