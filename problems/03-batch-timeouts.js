...
function batchTimeouts(callbacks, delays) {
  const timeoutObjs = []; // Array to store the timeout objects

  for (let i = 0; i < callbacks.length; i++) { // Loop through the callbacks array
    const timeout = setTimeout(callbacks[i], delays[i]); // Set a timeout for the current callback with the corresponding delay
    timeoutObjs.push(timeout); // Push the timeout object to the array
  }

  return timeoutObjs; // Return the array of timeout objects
}
